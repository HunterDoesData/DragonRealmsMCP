...
public:
    ...
    Tray* getTray();
    AiBridgeService* getAiBridgeService();
    void startAiNavigation(const QString& destination);
    void previewAiNavigation(const QString& destination);
    void startAiNavigationToRoom(const QString& zoneId, int level, int nodeId);
...
private:
    ...
    QMenu* commandMenu;
    QMap<int, QString> pendingAiCommands;
    int nextAiCommandId;
    QTimer* aiNavigationTimer;
