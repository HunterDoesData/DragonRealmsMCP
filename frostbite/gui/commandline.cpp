...
bool CommandLine::filterCommand(QString text) {
    if (text == "/aihelp") {
        mainWindow->getAiBridgeService()->showAssistantHelp();
        this->clear();
        return true;
    } else if (text == "/aistop") {
        mainWindow->stopAiNavigation();
        this->clear();
        return true;
    } else if (text == "/aigo" || text.startsWith("/aigo ")) {
        mainWindow->startAiNavigation(text.mid(QString("/aigo").length()).trimmed());
        this->clear();
        return true;
    } else if (text == "/airoute" || text.startsWith("/airoute ")) {
        mainWindow->previewAiNavigation(text.mid(QString("/airoute").length()).trimmed());
        this->clear();
        return true;
    } else if (text.startsWith("/aimapnav")) {
        const QString value = text.mid(QString("/aimapnav").length()).trimmed().toLower();
        if (value == "on") {
            mainWindow->setAiMapNavigationEnabled(true);
            mainWindow->handleAiBridgeStatus("Map-click navigation is enabled. Click a map room to route there.");
        } else if (value == "off") {
            mainWindow->setAiMapNavigationEnabled(false);
            mainWindow->handleAiBridgeStatus("Map-click navigation is disabled.");
        } else {
            const bool enabled = mainWindow->isAiMapNavigationEnabled();
            mainWindow->handleAiBridgeStatus(QString("Map-click navigation is %1.").arg(enabled ? "enabled" : "disabled"));
        }
        this->clear();
        return true;
    } else if (text == "/aipending") {
        mainWindow->listAiPendingCommands();
        this->clear();
        return true;
    } else if (text == "/aiapprove" || text.startsWith("/aiapprove ")) {
        bool ok = false;
        const int id = text.mid(QString("/aiapprove").length()).trimmed().toInt(&ok);
        mainWindow->approveAiCommand(ok ? id : -1);
        this->clear();
        return true;
    } else if (text == "/aireject" || text.startsWith("/aireject ")) {
        bool ok = false;
        const int id = text.mid(QString("/aireject").length()).trimmed().toInt(&ok);
        mainWindow->rejectAiCommand(ok ? id : -1);
        this->clear();
        return true;
    } else if (text.startsWith("/aiobserve")) {
        const QString value = text.mid(QString("/aiobserve").length()).trimmed().toLower();
        if (value == "on") {
            mainWindow->getAiBridgeService()->setObserverMode(true);
        } else if (value == "off") {
            mainWindow->getAiBridgeService()->setObserverMode(false);
        } else {
        ...
        this->clear();
        return true;
    } else if (text == "/aiwiki" || text.startsWith("/aiwiki ")) {
        mainWindow->getAiBridgeService()->requestElanthipediaSummary(text.mid(QString("/aiwiki").length()).trimmed());
        this->clear();
        return true;
    } else if (text == "/aimodel") {
        mainWindow->getAiBridgeService()->showAssistantModel();
        this->clear();
        return true;
    } else if (text == "/ai" || text.startsWith("/ai ")) {
        mainWindow->getAiBridgeService()->requestAssistantResponse(text.mid(3).trimmed());
        this->clear();
        return true;
